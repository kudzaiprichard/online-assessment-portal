<?php
    ...
    if (isset($_GET['first_name'])) {
        ...
        $supervisor = stripslashes($_REQUEST['supervisor']);    
        $supervisor = (int) mysqli_real_escape_string($con, $supervisor);
        

        $assessor = stripslashes($_REQUEST['assessor']);    
        $assessor = (int) mysqli_real_escape_string($con, $assessor);

        // echo var_dump($supervisor); 
        // echo $supervisor;
        // exit();
        
        $adminController->addStudent($firstName, $lastName, $regNumber, $program, $phoneNumber, $emailAddress, $physicalAddress,$assessor,$supervisor);
