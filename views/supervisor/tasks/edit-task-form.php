...
if(isset($_GET['id'])){
    $taskId = $_GET['id'];
    $task = $supervisorController->fetchTaskById($taskId);
}
?>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card-style mb-30">
                    <form class="row g-3 needs-validation" action="update-task.php" novalidate>
                        <div class="modal-body">
                            <div class="col m-2 p-2">
                                <label for="validationCustom01" class="form-label">Task Name</label>
                                <?php echo'<input type="text" class="form-control" name="name" id="" aria-describedby="helpId" placeholder="'.$task->getName().'" value="'.$task->getName().'">';?>
                            </div>
                            <div class="col m-2 p-2">
                                <label for="validationCustom01" class="form-label">Task Summary</label>
                                <?php echo '<textarea class="form-control" name="summary" id="" rows="3" placeholder="'.$task->getDescription().'">'.$task->getDescription().'</textarea>';?>
                            </div>
                            <div class="col m-2 p-2">
                                <label for="validationCustom01" class="form-label">Comment</label>
                                <?php echo'<textarea class="form-control" name="comment" id="" rows="3" placeholder="'.$task->getSupervisorComment().'" required>'.$task->getSupervisorComment().'</textarea>';?>
                            </div>
                            <div class="col m-2 p-2">
                                <label for="validationCustom01" class="form-label">Rate</label>
                                <select class="form-select form-select-sm" name="rate" id="">
                                    <?php echo'<option value="'.$task->getRating().'" selected>'.$task->getRating().'</option>';?>
